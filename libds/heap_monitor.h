 ...
 */
template <typename T> T *placement_new(T *address) { return new (address) T(); }

/**
 * @brief Creates copy-constructed object from @p origin at @p address.
 */
template <typename T> T *placement_copy(T *address, const T &origin) {
 ...
 */
template <typename T> void destroy(T *address) { address->~T(); }
