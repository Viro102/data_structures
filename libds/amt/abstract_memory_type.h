...
template <typename DataType> struct MemoryBlock {
...
template <typename BlockType> class AbstractMemoryStructure : virtual public AMT {
  public:
...
template <typename BlockType> using AMS = AbstractMemoryStructure<BlockType>;
...
template <typename DataType> using ImplicitAMS = ImplicitAbstractMemoryStructure<DataType>;

//----------

template <typename BlockType> class ExplicitAbstractMemoryStructure : public AMS<BlockType> {
  public:
...
template <typename BlockType> using ExplicitAMS = ExplicitAbstractMemoryStructure<BlockType>;

//----------

template <typename BlockType> class Iterator {
  public:
    virtual ~Iterator() {}
...
template <typename BlockType> class ForwardIterator : public Iterator<BlockType> {
  public:
...
template <typename BlockType> class BidirectionalIterator : public ForwardIterator<BlockType> {
  public:
...
template <typename BlockType> class RandomAccessIterator : public BidirectionalIterator<BlockType> {
  public:
...
template <typename BlockType> class CompactMemoryIterator : public Iterator<BlockType> {
  public:
    CompactMemoryIterator(ImplicitAMS<BlockType> *structure, size_t current)
        : structure_(structure), current_(current) {}
...
template <typename BlockType> class NonCompactMemoryIterator : public Iterator<BlockType> {
  public:
    NonCompactMemoryIterator(BlockType *current) : current_(current) {}

    ~NonCompactMemoryIterator() override { current_ = nullptr; }

    BlockType &accessData() const override { return *current_; }
...
template <typename BlockType> AbstractMemoryStructure<BlockType>::~AbstractMemoryStructure() {
    delete memoryManager_;
    memoryManager_ = nullptr;
}

template <typename BlockType> size_t AbstractMemoryStructure<BlockType>::size() const {
    return memoryManager_->getAllocatedBlockCount();
}

template <typename BlockType> bool AbstractMemoryStructure<BlockType>::isEmpty() const {
    return this->size() == 0;
}
...
template <typename DataType>
ImplicitAbstractMemoryStructure<DataType>::ImplicitAbstractMemoryStructure(size_t capacity,
                                                                           bool initBlocks)
...
template <typename DataType>
ImplicitAbstractMemoryStructure<DataType>::ImplicitAbstractMemoryStructure(
    mm::CompactMemoryManager<BlockType> *memoryManager)
...
AMT &ImplicitAbstractMemoryStructure<DataType>::assign(const AMT &other) {
    if (this != &other) {
        const ImplicitAMS<DataType> &otherImplicitStructure =
            dynamic_cast<const ImplicitAMS<DataType> &>(other);
...
template <typename DataType> void ImplicitAbstractMemoryStructure<DataType>::clear() {
...
template <typename DataType> size_t ImplicitAbstractMemoryStructure<DataType>::getCapacity() {
