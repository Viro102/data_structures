...
TestOutputterVisitor::TestOutputterVisitor(ConsoleOutputType o) : otype_(o) {}

auto TestOutputterVisitor::visit(LeafTest &t) -> void {
    if (prefix_.empty()) {
...
auto TestOutputterVisitor::visit(CompositeTest &t) -> void {
    if (prefix_.empty()) {
        console_.println(t.name(), test_result_to_color(t.result()));
    }
