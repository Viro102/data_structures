...
Test::Test(std::string name) : name_(std::move(name)) {}

auto Test::name() const -> std::string_view { return name_; }
...
LeafTest::LeafTest(std::string name, AssertPolicy policy)
    : Test(std::move(name)), assertPolicy_(policy) {}
...
auto LeafTest::result() const -> TestResult {
    ...
    std::copy_if(begin(results_), end(results_), std::back_inserter(asserts),
                 [](TestMessage const &m) { return m.type_ != TestMessageType::Info; });

    auto const is_pass = [](TestMessage const &m) { return m.type_ == TestMessageType::Pass; };

    return results_.empty()                                      ? TestResult::NotEvaluated
           : std::all_of(begin(asserts), end(asserts), is_pass)  ? TestResult::Pass
           : std::none_of(begin(asserts), end(asserts), is_pass) ? TestResult::Fail
                                                                 : TestResult::Partial;
}

auto LeafTest::reset() -> void { results_.clear(); }

auto LeafTest::output() const -> std::vector<TestMessage> const & { return results_; }

auto LeafTest::accept(IVisitor &v) -> void { v.visit(*this); }

auto LeafTest::assert_true(bool const b, std::string m) -> void {
    if (b) {
...
auto LeafTest::assert_false(bool const b, std::string m) -> void {
    this->assert_true(not b, std::move(m));
}

auto LeafTest::assert_null(std::nullptr_t) -> void { this->pass("Null literal is indeed null"); }
...
auto LeafTest::info(std::string m) -> void {
    results_.emplace_back(TestMessage{TestMessageType::Info, std::move(m)});
}
...
auto LeafTest::pass(std::string m) -> void {
    results_.emplace_back(TestMessage{TestMessageType::Pass, std::move(m)});
}

auto LeafTest::log_fail(std::string m) -> void {
    results_.emplace_back(TestMessage{TestMessageType::Fail, std::move(m)});
}
...
auto CompositeTest::accept(IVisitor &v) -> void { v.visit(*this); }
...
auto CompositeTest::result() const -> TestResult {
    ...
    std::transform(begin(tests_), end(tests_), std::back_inserter(results),
                   [](std::unique_ptr<Test> const &test) { return test->result(); });

    auto const didfail = [](TestResult const r) { return r == TestResult::Fail; };
    auto const didpass = [](TestResult const r) { return r == TestResult::Pass; };
    auto const didntev = [](TestResult const r) { return r == TestResult::NotEvaluated; };

    return std::all_of(begin(results), end(results), didntev)   ? TestResult::NotEvaluated
           : std::all_of(begin(results), end(results), didfail) ? TestResult::Fail
           : std::all_of(begin(results), end(results), didpass) ? TestResult::Pass
                                                                : TestResult::Partial;
}
...
auto CompositeTest::subtests() const -> std::vector<std::unique_ptr<Test>> const & {
    return tests_;
}

auto CompositeTest::subtests() -> std::vector<std::unique_ptr<Test>> & { return tests_; }

auto CompositeTest::add_test(std::unique_ptr<Test> t) -> void { tests_.emplace_back(std::move(t)); }
...
DummyData::DummyData() : number_(new int()) {}

DummyData::DummyData(int number) : number_(new int(number)) {}
...
auto DummyData::get_number() const -> int { return *number_; }

auto DummyData::set_number(int newNumber) -> void { *number_ = newNumber; }
} // namespace ds::tests
