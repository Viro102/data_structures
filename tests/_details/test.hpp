 ...
 */
enum class TestResult { Pass, Fail, Partial, NotEvaluated };

/**
 *  @brief Type of a test message.
 */
enum class TestMessageType { Pass, Fail, Info };
 ...
 */
enum class AssertPolicy { StopAtFirstFail, RunAll };
...
class LeafTest : public Test {
  public:
     ...
     */
    LeafTest(std::string name, AssertPolicy policy = AssertPolicy::StopAtFirstFail);
  ...
  protected:
     ...
     */
    template <class T>
    auto assert_equals(T expected, T actual) -> std::enable_if_t<!std::is_floating_point_v<T>>;
     ...
     */
    template <class T>
    auto assert_not_equals(T expected, T actual) -> std::enable_if_t<!std::is_floating_point_v<T>>;
     ...
     */
    template <class F> auto assert_throws(F f) -> std::enable_if_t<std::is_invocable_v<F>>;
     ...
     */
    template <class F>
    auto assert_throws(F f, std::string message) -> std::enable_if_t<std::is_invocable_v<F>>;
     ...
     *  @param p pointer
     */
    template <class T> auto assert_null(T *p) -> void;

    /**
     *  @brief Asserts that @p is nullptr.
     *  @param p pointer
     */
    template <class T, class D> auto assert_null(std::unique_ptr<T, D> const &p) -> void;

    /**
     *  @brief Asserts that @p is nullptr.
     *  @param p pointer
     */
    template <class T> auto assert_null(std::shared_ptr<T> const &p) -> void;
     ...
     *  @param p pointer
     */
    template <class T> auto assert_not_null(T *p) -> void;

    /**
     *  @brief Asserts that @p p is not nullptr.
     *  @param p pointer
     */
    template <class T, class D> auto assert_not_null(std::unique_ptr<T, D> const &p) -> void;

    /**
     *  @brief Asserts that @p p is not nullptr.
     *  @param p pointer
     */
    template <class T> auto assert_not_null(std::shared_ptr<T> const &p) -> void;
     ...
     *  @param o optional
     */
    template <class T> auto assert_nullopt(std::optional<T> const &o) -> void;
     ...
     *  @param o optional
     */
    template <class T> auto assert_has_value(std::optional<T> const &o) -> void;
 ...
 */
auto console_print_results(Test &t, ConsoleOutputType o = ConsoleOutputType::Full) -> void;
...
auto LeafTest::assert_equals(T expected, T actual)
    ...
    if constexpr (std::is_arithmetic_v<T>) {
        ...
        this->assert_equals(expected, actual, "Expected " + expected + " got " + actual);
    } else {
...
auto LeafTest::assert_equals(T expected, T actual, T epsilon)
    ...
    ost << "Expected " << expected << " got " << actual << " using presision " << epsilon;
...
auto LeafTest::assert_not_equals(T expected, T actual)
    ...
    if constexpr (std::is_arithmetic_v<T>) {
        ...
        this->assert_not_equals(expected, actual, "Expected " + expected + " got " + actual);
    } else {
...
auto LeafTest::assert_not_equals(T expected, T actual, T epsilon)
    ...
    ost.precision(std::numeric_limits<double>::max_digits10);
    ost << "Expected " << expected << " and " << actual << " to be different using presision "
        << epsilon;
...
template <class F> auto LeafTest::assert_throws(F f) -> std::enable_if_t<std::is_invocable_v<F>> {
    this->assert_throws(f, "Function throws");
}

template <class F>
auto LeafTest::assert_throws(F f, std::string m) -> std::enable_if_t<std::is_invocable_v<F>> {
    try {
        std::invoke(f);
...
template <class T> auto LeafTest::assert_null(T *p) -> void {
    this->assert_true(!p, "Pointer is nullptr");
}

template <class T, class D> auto LeafTest::assert_null(std::unique_ptr<T, D> const &p) -> void {
    this->assert_true(!p.get(), "Pointer is nullptr");
}

template <class T> auto LeafTest::assert_null(std::shared_ptr<T> const &p) -> void {
    this->assert_true(!p.get(), "Pointer is nullptr");
}

template <class T> auto LeafTest::assert_not_null(T *p) -> void {
    this->assert_true(p, "Pointer is not nullptr");
}

template <class T, class D> auto LeafTest::assert_not_null(std::unique_ptr<T, D> const &p) -> void {
    this->assert_true(p.get(), "Pointer is not nullptr");
}

template <class T> auto LeafTest::assert_not_null(std::shared_ptr<T> const &p) -> void {
    this->assert_true(p.get(), "Pointer is not nullptr");
}

template <class T> auto LeafTest::assert_nullopt(std::optional<T> const &o) -> void {
    this->assert_true(!o.has_value(), "Optional is nullopt");
}

template <class T> auto LeafTest::assert_has_value(std::optional<T> const &o) -> void {
