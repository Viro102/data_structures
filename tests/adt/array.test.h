...
class ArrayTestSetAccess : public LeafTest {
  ...
  protected:
    void test() override {
        ...
        this->assert_throws([=, &arr]() { arr.set(-1, base + static_cast<int>(size) + 10); });

        this->assert_throws([=, &arr]() { (void)arr.access(base + static_cast<int>(size) + 10); });
    }
};
...
class MatrixTestSetAccess : public LeafTest {
  ...
  protected:
    void test() override {
        ...
        for (long long i = d1.getBase(); i < d1.getBase() + static_cast<long long>(d1.getSize());
             ++i) {
            for (long long j = d2.getBase();
                 j < d2.getBase() + static_cast<long long>(d2.getSize()); ++j) {
        ...
        this->assert_throws([&]() { matrix.set(-1, d1.getBase() - 10, d2.getBase() + 1); });

        this->assert_throws([&]() { matrix.set(-1, d1.getBase() + 1, d2.getBase() - 10); });

        this->assert_throws([&]() { matrix.set(-1, d1.getBase() - 10, d2.getBase() - 10); });

        this->assert_throws([&]() { (void)matrix.access(d1.getBase() - 10, d2.getBase() + 1); });

        this->assert_throws([&]() { (void)matrix.access(d1.getBase() + 1, d2.getBase() - 10); });

        this->assert_throws([&]() { (void)matrix.access(d1.getBase() - 10, d2.getBase() - 10); });
    }
};
...
class MatrixTestCopyAssignEquals : public LeafTest {
  ...
  protected:
    void test() override {
        ...
        this->assert_throws([&]() {
            auto matrix4 =
                adt::CompactMatrix<int>(adt::Dimension(d1.getBase() + 1, d1.getSize() - 1), d2);
            matrix4.assign(matrix3);
        });

        this->assert_throws([&]() {
            auto matrix4 =
                adt::CompactMatrix<int>(d1, adt::Dimension(d2.getBase() + 1, d2.getSize() - 1));
            matrix4.assign(matrix3);
        });
    }
};
