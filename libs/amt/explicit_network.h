...
#include "implicit_sequence.h"
#include "network.h"
#include <functional>

namespace ds::amt {
template <typename DataType> struct NetworkBlockImplicitRelations : public MemoryBlock<DataType> {
    using RelationBlockType =
        typename ImplicitAMS<NetworkBlockImplicitRelations<DataType> *>::BlockType;

    NetworkBlockImplicitRelations()
        : relations_(new IS<NetworkBlockImplicitRelations<DataType> *>()) {}
...
template <typename DataType> using IRNetworkBlock = NetworkBlockImplicitRelations<DataType>;

template <typename DataType> struct NetworkBlockExplicitRelations : public MemoryBlock<DataType> {
    using RelationBlockType =
        typename DoublyLS<NetworkBlockExplicitRelations<DataType> *>::BlockType;

    NetworkBlockExplicitRelations()
        : relations_(new DoublyLS<NetworkBlockExplicitRelations<DataType> *>()) {}
...
template <typename DataType> using ERNetworkBlock = NetworkBlockExplicitRelations<DataType>;
...
template <typename DataType> using IGIRNetwork = ImplicitGateImplicitRelationsNetwork<DataType>;

template <typename DataType>
class ImplicitGateExplicitRelationsNetwork
    : public ExplicitNetwork<ERNetworkBlock<DataType>, IS<ERNetworkBlock<DataType> *>> {};

template <typename DataType> using IGERNetwork = ImplicitGateExplicitRelationsNetwork<DataType>;
...
template <typename DataType> using EGIRNetwork = ExplicitGateImplicitRelationsNetwork<DataType>;
...
template <typename DataType> using EGERNetwork = ExplicitGateExplicitRelationsNetwork<DataType>;
...
AMT &ExplicitNetwork<BlockType, GateType>::assign(const AMT &other) {
    if (this != &other) {
        ...
        const ExplicitNetwork<BlockType, GateType> &otherExplicitNetwork =
            dynamic_cast<const ExplicitNetwork<BlockType, GateType> &>(other);

        otherExplicitNetwork.gate_->processAllBlocksForward(
            [&](GateBlockType *b) { insert().data_ = b->data_->data_; });
        ...
        while (otherBlockFrom != nullptr) {
            otherBlockFrom->data_->relations_->processAllBlocksForward(
                [&](RelationsBlockType *otherRelationsBlock) {
                    size_t otherGateIndexTo = 0;
                    otherExplicitNetwork.gate_->findBlockWithProperty(
                        [&](GateBlockType *otherGateBlockIndexTo) -> bool {
...
bool ExplicitNetwork<BlockType, GateType>::equals(const AMT &other) {
    ...
    while (myGateBlock != nullptr) {
        if (myGateBlock->data_ != otherGateBlock->data_) {
            return false;
        }
        if (myGateBlock->data_->relations_->size() != otherGateBlock->data_->relations_->size()) {
            return false;
        }
        ...
        while (myRelationsBlock != nullptr) {
            if (myRelationsBlock->data_ != otherRelationsBlock->data_) {
                return false;
            }

            myRelationsBlock = myGateBlock->data_->relations_->accessNext(*myRelationsBlock);
            otherRelationsBlock =
                otherGateBlock->data_->relations_->accessNext(*otherRelationsBlock);
        }
...
size_t ExplicitNetwork<BlockType, GateType>::relationCount() const {
    size_t result = 0;
    gate_->processAllBlocksForward(
        [&result](const GateBlockType *b) { result += b->data_->relations_->size(); });
    return result;
}
...
template <typename BlockType, typename GateType>
BlockType *ExplicitNetwork<BlockType, GateType>::accessNodeFromNode(const BlockType &node,
                                                                    size_t order) const {
    return node.relations_->access(order)->data_;
}

template <typename BlockType, typename GateType>
bool ExplicitNetwork<BlockType, GateType>::relationExists(const BlockType &nodeA,
                                                          const BlockType &nodeB) const {
    ...
    relationExists = [](const BlockType &vrcholOd, const BlockType &vrcholDo) -> bool {
        return vrcholOd.relations_->findBlockWithProperty(
                   [&vrcholDo](RelationsBlockType *b) -> bool { return b->data_ == &vrcholDo; }) !=
               nullptr;
    };

    return degree(nodeA) <= degree(nodeB) ? relationExists(nodeA, nodeB)
                                          : relationExists(nodeB, nodeA);
}
...
void ExplicitNetwork<BlockType, GateType>::remove(BlockType *node) {
    ...
    if (gateBlock->data_ == node) {
        ...
        GateBlockType *prevGateBlock = gate_->findPreviousToBlockWithProperty(
            [&node](GateBlockType *b) -> bool { return b->data_ == node; });
...
void ExplicitNetwork<BlockType, GateBlock>::disconnect(BlockType &nodeA, BlockType &nodeB) {
    ...
    disconnectRelation = [](const BlockType &nodeFrom, const BlockType &nodeTo) {
        if (nodeFrom.relations_->accessFirst()->data_ == &nodeTo) {
            ...
            RelationsBlockType *prevInRelationsFrom =
                nodeFrom.relations_->findPreviousToBlockWithProperty(
                    [&nodeTo](RelationsBlockType *b) -> bool { return b->data_ == &nodeTo; });
...
template <typename BlockType, typename GateType>
typename ExplicitNetwork<BlockType, GateType>::IteratorType
ExplicitNetwork<BlockType, GateType>::begin() {
    return gate_->begin();
}

template <typename BlockType, typename GateType>
typename ExplicitNetwork<BlockType, GateType>::IteratorType
ExplicitNetwork<BlockType, GateType>::end() {
    return gate_->end();
}
} // namespace ds::amt
