...
class ImplicitHierarchy : virtual public KWayHierarchy<MemoryBlock<DataType>, K>,
  ...
  public:
    ...
    // Overload with one parameter hides overload with no parameter. We need to explicitly 'include'
    // it.
    ...
    MemoryBlock<DataType> &emplaceSon(MemoryBlock<DataType> &parent, size_t sonOrder) override;

    // throw(unavailable_function_call)
    void changeSon(MemoryBlock<DataType> &parent, size_t sonOrder,
                   MemoryBlock<DataType> *newSon) override;
...
template <typename DataType, size_t K> using IH = ImplicitHierarchy<DataType, K>;
...
template <typename DataType, size_t K> ImplicitHierarchy<DataType, K>::ImplicitHierarchy() {}
...
size_t ImplicitHierarchy<DataType, K>::level(size_t index) const {
    return floor(log((K - 1) * (index + 1)) / log(K));
}
...
size_t ImplicitHierarchy<DataType, K>::degree(size_t index) const {
    int currentLevel = this->level(index);
    int indexOfPrevious = this->size() - 1;
    int depth = this->level(indexOfPrevious);

    if (currentLevel == depth) {
        return 0;
    } else if (currentLevel == depth - 1) {
        int indexOfPreviousParent = this->indexOfParent(indexOfPrevious);
        if (index < indexOfPreviousParent) {
            return K;
        } else if (index > indexOfPreviousParent) {
            return 0;
        } else {
            int mod = (this->size() - 1) % K;
            return mod == 0 ? K : mod;
        }
    } else {
        return K;
...
ImplicitHierarchy<DataType, K>::accessParent(const MemoryBlock<DataType> &node) const {
    int index = this->indexOfParent(node);

    return INVALID_INDEX != index ? &this->getMemoryManager()->getBlockAt(index) : nullptr;
}

template <typename DataType, size_t K>
MemoryBlock<DataType> *ImplicitHierarchy<DataType, K>::accessSon(const MemoryBlock<DataType> &node,
                                                                 size_t sonOrder) const {
    int index = this->indexOfSon(node, sonOrder);
...
template <typename DataType, size_t K>
MemoryBlock<DataType> &ImplicitHierarchy<DataType, K>::emplaceSon(MemoryBlock<DataType> &parent,
                                                                  size_t sonOrder) {
    throw unavailable_function_call("Method emplaceSon() unavailable in implicit hierarchies!");
}

template <typename DataType, size_t K>
void ImplicitHierarchy<DataType, K>::changeSon(MemoryBlock<DataType> &parent, size_t sonOrder,
...
template <typename DataType, size_t K> void ImplicitHierarchy<DataType, K>::removeLastLeaf() {
...
template <typename DataType, size_t K>
size_t ImplicitHierarchy<DataType, K>::indexOfSon(const MemoryBlock<DataType> &node,
                                                  size_t sonOrder) const {
