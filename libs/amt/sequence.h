...
namespace ds::amt {
template <typename BlockType> class Sequence : virtual public AMT {
  public:
    ...
    virtual void processBlocksForward(BlockType *block,
                                      std::function<void(BlockType *)> operation) const;
    virtual void processBlocksBackward(BlockType *block,
                                       std::function<void(BlockType *)> operation) const;
...
template <typename BlockType>
void Sequence<BlockType>::processBlocksForward(BlockType *block,
                                               std::function<void(BlockType *)> operation) const {
    while (block != nullptr) {
        operation(block);
...
template <typename BlockType>
void Sequence<BlockType>::processBlocksBackward(BlockType *block,
                                                std::function<void(BlockType *)> operation) const {
    while (block != nullptr) {
        operation(block);
...
template <typename BlockType>
BlockType *
Sequence<BlockType>::findBlockWithProperty(std::function<bool(BlockType *)> predicate) const {
...
BlockType *Sequence<BlockType>::findPreviousToBlockWithProperty(
    std::function<bool(BlockType *)> predicate) const {
