      ...
      protected:
        struct DepthFirstIteratorPosition {
            DepthFirstIteratorPosition(BlockType *currentNode,
                ...
                : currentNode_(currentNode), currentSon_(nullptr), currentSonOrder_(INVALID_INDEX),
                  visitedSonCount_(0), currentNodeProcessed_(false),
...
template <typename BlockType> class BinaryHierarchy : virtual public KWayHierarchy<BlockType, 2> {
  public:
...
template <typename BlockType> size_t Hierarchy<BlockType>::nodeCount() const {
    size_t result = 0;
    this->processPreOrder(this->accessRoot(), [&result](const BlockType *b) { result++; });
    return result;
}

template <typename BlockType> size_t Hierarchy<BlockType>::nodeCount(const BlockType &node) const {
    size_t result = 0;
...
template <typename BlockType>
void Hierarchy<BlockType>::processLevelOrder(BlockType *node,
                                             std::function<void(BlockType *)> operation) const {
    if (node != nullptr) {
...
Hierarchy<BlockType>::DepthFirstIterator::DepthFirstIterator(const DepthFirstIterator &other)
    ...
    for (DepthFirstIteratorPosition *otherPosition = other.currentPosition_;
         otherPosition != nullptr; otherPosition = otherPosition->previousPosition_) {
...
template <typename BlockType> Hierarchy<BlockType>::DepthFirstIterator::~DepthFirstIterator() {
...
template <typename BlockType> void Hierarchy<BlockType>::DepthFirstIterator::removePosition() {
...
template <typename BlockType> void BinaryHierarchy<BlockType>::removeLeftSon(BlockType &parent) {
    this->removeSon(parent, LEFT_SON_INDEX);
}

template <typename BlockType> void BinaryHierarchy<BlockType>::removeRightSon(BlockType &parent) {
...
bool BinaryHierarchy<BlockType>::InOrderHierarchyIterator::tryToGoToLeftSonInCurrentPosition() {
    this->currentPosition_->currentSon_ = this->hierarchy_->accessSon(
        *this->currentPosition_->currentNode_, BinaryHierarchy<BlockType>::LEFT_SON_INDEX);
...
bool BinaryHierarchy<BlockType>::InOrderHierarchyIterator::tryToGoToRightSonInCurrentPosition() {
    this->currentPosition_->currentSon_ = this->hierarchy_->accessSon(
        *this->currentPosition_->currentNode_, BinaryHierarchy<BlockType>::RIGHT_SON_INDEX);
    if (this->currentPosition_->currentSon_ != nullptr) {
