...
template <typename T> class AbstractDataStructure : virtual public ADT {
  public:
...
template <typename T> using ADS = AbstractDataStructure<T>;

//----------

template <typename T> class AbstractDataMultistructure : virtual public ADT {};

template <typename T> using AUMS = AbstractDataMultistructure<T>;
...
template <typename T> AbstractDataStructure<T>::~AbstractDataStructure() {
    delete memoryStructure_;
}

template <typename T> ADT &AbstractDataStructure<T>::assign(const ADT &other) {
    if (this != &other) {
...
template <typename T> void AbstractDataStructure<T>::clear() { memoryStructure_->clear(); }

template <typename T> size_t AbstractDataStructure<T>::size() const {
    return memoryStructure_->size();
}

template <typename T> bool AbstractDataStructure<T>::isEmpty() const { return this->size() == 0; }

template <typename T> bool AbstractDataStructure<T>::equals(const ADT &other) {
    if (this != &other) {
        const ADS<T> &otherADS = dynamic_cast<const ADS<T> &>(other);
