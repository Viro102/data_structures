...
ListAnalyzer<List>::ListAnalyzer(const std::string &name)
    ...
    this->registerAfterOperation([](List &list) { list.clear(); });
}
...
template <class List> size_t ListAnalyzer<List>::getRandomIndex() const { return index_; }

template <class List> int ListAnalyzer<List>::getRandomData() const { return data_; }
...
template <class List> void ListInsertAnalyzer<List>::executeOperation(List &structure) {
