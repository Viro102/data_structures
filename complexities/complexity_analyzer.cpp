...
const std::string &Analyzer::getName() const { return name_; }
...
void LeafAnalyzer::setReplicationCount(size_t count) { replicationCount_ = count; }

void LeafAnalyzer::setStepSize(size_t size) { stepSize_ = size; }

void LeafAnalyzer::setStepCount(size_t count) { stepCount_ = count; }

std::filesystem::path LeafAnalyzer::getOutputPath() const {
    return absolute(std::filesystem::path(outputDir_) / this->getName() += ".csv");
}

bool LeafAnalyzer::wasSuccessful() const { return wasSuccessful_; }

void LeafAnalyzer::resetSuccess() { wasSuccessful_ = false; }

void LeafAnalyzer::setSuccess() { wasSuccessful_ = true; }

size_t LeafAnalyzer::getReplicationCount() const { return replicationCount_; }

size_t LeafAnalyzer::getStepSize() const { return stepSize_; }

size_t LeafAnalyzer::getStepCount() const { return stepCount_; }
} // namespace ds::utils
